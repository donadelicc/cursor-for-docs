...
const CollapsiblePanel: React.FC<CollapsiblePanelProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={`flex-shrink-0 px-4 py-3 ${isOpen ? 'bg-white border-b border-gray-200' : 'bg-white'}`}>
        {isOpen ? (
          ...
          <div className="flex items-center justify-between">
            {position === 'left' ? (
              <>
                ...
                <button
                  onClick={togglePanel}
                  className="p-2 rounded-md hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition-all duration-200"
                  title={`Close ${title}`}
                >
                  <svg
                    className="w-6 h-6"
          ...
          <button
            onClick={togglePanel}
            className="w-full flex items-center justify-center py-2 bg-transparent hover:bg-gray-100 text-gray-600 hover:text-gray-800 rounded-md transition-all duration-200"
            title={`Open ${title}`}
          >
            {position === 'left' ? (
              <svg
