  ...
  try {
    ...
    const docDefinition = {
      content:
        content.length > 0
          ? content
          : [{ text: "No content available.", style: "normal" }],
      styles: {
        header1: {
          fontSize: 20,
          bold: true,
    ...
    if (error instanceof Error) {
      if (error.message.includes("vfs")) {
        throw new Error(
          "PDF font loading failed. This might be due to a PDFMake version issue. The document structure will be preserved but fonts may appear different.",
        );
      } else {
        throw new Error(
          `Failed to convert document to PDF format: ${error.message}`,
        );
      }
    } else {
      throw new Error(
        "Failed to convert document to PDF format: Unknown error occurred",
      );
    }
  }
}
  ...
  switch (tagName) {
    ...
    case "ul":
      ...
      for (const listItem of ulListItems) {
        ...
        ulItems.push(
          runs.length > 0 ? runs : { text: listItem.textContent || "" },
        );
      }
      if (ulItems.length > 0) {
        elements.push({
